...
function updateCount(){
  document.getElementById("count").innerText = getAdminQuestions().length;
}
...
function clearResults(){
  ...
  let confirmDelete = confirm(
    "Are you sure you want to delete all student results?"
  );
  ...
  if(confirmDelete){
    ...
    alert("All student results deleted");
...
function showLeaderboard(){
  ...
  if(results.length === 0){
    box.innerHTML = "<p>No leaderboard data found.</p>";
    return;
  }
...
function convertPDFTextToQuestions() {
  ...
  let questionBlocks = cleanedText.split(/Q\.\s*\d+|Question\s*\d+|\b\d+\)/i).slice(1);
...
function exportResults(){
  ...
  let csv =
    "Name,Total Questions,Attempted,Correct,Wrong,Score,Date\n";

  results.forEach(result => {

    csv +=
      `${result.name},${result.totalQuestions},${result.attempted},${result.correct},${result.wrong},${result.score},${result.date}\n`;
    ...
    allTests.forEach((test, index) => {
      html += `
        <tr>
          ...
          <td>
          <button onclick="editMockTest('${test._id}')">
  Edit
</button>
